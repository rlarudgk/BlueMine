		...
		<div class="modal-content" id="projectWorkSelectDetailDisp">
			...
			<div class="modal-body row">
				<div class="col-md-12 col-xl-12">
					...
					<input type="hidden" id="alarmCd" value="AC005">
				</div>
			</div>
...
<script type="text/javascript">
	...
	$(".projectWorkInsertBtn").on("click", function(){
	    ...
	    for(let i=0; i<workList.length; i++){
	    	let objOption = document.createElement("option");
		    objOption.text = "["+ workList[i].workNo + "] " + workList[i].workTitle;
...
// 		// 알람 리스트에 추가
	...
	$(document).on("click", ".isWorkRemoveBtn", function(){
		if($("div").hasClass("isWorkMemberRemoveBtn")) {
			...
			// 알람 리스트에서 삭제
			let removeMem = $(this).parent().find("span").html();
			for(let i = 0; i < mailList.length; i++) {
	           if(mailList[i] === removeMem)  {
	              mailList.splice(i, 1);
	               i--;
	           }
	         }
