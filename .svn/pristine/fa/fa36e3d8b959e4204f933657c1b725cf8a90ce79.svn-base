...
public class NewsServiceImpl implements NewsService{
	...
	public ServiceResult createNews(NewsVO news) {
		...
		//파일첨부
		MultipartFile[] files = news.getFiles(); 
		if(files!=null && files.length > 0) {
		   int newsAttNo = attService.createAttatch(files,"newsFile");
		   news.setNewsAttNo(newsAttNo);
		}
	...
	//댓글 작성 	
	@Override
	public int createNewsComm(NewsCommVO newscommVo) {
		
		int rowcnt = newsDAO.insertNewsComm(newscommVo);
		
		return rowcnt;
	}

	@Override
	public List<NewsCommVO> retriveNewsCommList(NewsCommVO newscommVo) {
		return newsDAO.selectNewsCommList(newscommVo);
	}
}
