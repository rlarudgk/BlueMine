...
public class WorkController {
	...
	// 작업 추가 Form HTMl 가져오기
	@GetMapping("/project/workAddForm/{proCode}")
	public String projectWorkAddForm(
			@PathVariable String proCode
			, Model model){
		ProjectVO project = service.retrieveProject(proCode);
		model.addAttribute("project", project);
		return "project/work/workAddForm";
	}
	
	// 작업 추가 Form HTMl 가져오기
	@GetMapping("/notiles/workAddForm/{proCode}")
	public String notilesProjectWorkAddForm(
			@PathVariable String proCode
			, Model model){
		ProjectVO project = service.retrieveProject(proCode);
		model.addAttribute("project", project);
	...
	// 상위 작업 목록 가져오기
	@ResponseBody
	@GetMapping("/project/pntUpWorkListView/{proCode}")
	public List<WorkVO> projectPntUpWorkList(
			@PathVariable String proCode
			){
		List<WorkVO> workList = workService.retrievePntUpWorkList(proCode);
		return workList;
	}
	...
	// 작업 나에게 담당된 모든 프로젝트 전체 작업 목록 가져오기
	@ResponseBody
	@PostMapping("/project/allMyWorkListView")
	public List<WorkVO> projectAllMyWorkList(
			@RequestBody WorkVO work			
			){
		List<WorkVO> workList = workService.retrieveAllMyWorkList(work);
		return workList;
	}
