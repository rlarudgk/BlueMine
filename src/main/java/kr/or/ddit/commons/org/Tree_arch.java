...
//추상클래스를 상속받은 클래스를 만든다. 계층형 쿼리결과를 이용하여 트리구조를 만든다.
    		...
    		for(i=0;i<h;i++){
    			...
    			if (i!=0) {
    				...
    				if( ((TreeDTO)select.get(i)).getOrglevel() == menu_level ) {                       
    					menuString.append("<span id=SG"+i+"><IMG SRC='image/minus.png' BORDER=0 ID=IMG"+i+">");
    					...
    					menuString.append("<span id=SG"+i+"><A HREF=\""+javascript_1+"\"><IMG SRC='image/plus.png' BORDER=0 ID=IMG"+i+"></A>"
    					...
    					menuString.append("<span id=SG"+i+"><IMG SRC='image/minus.png' BORDER=0 ID=IMG"+i+"><A HREF=\""+javascript_2+"\" ID=A"+i+" class=fldblue> "+ menu_nm +"</A></span><br> \r");
